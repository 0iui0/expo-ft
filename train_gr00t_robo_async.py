#! /usr/bin/env python
"""EXPO-FT online RL training with GR00T N1.7 VLA actor — async learner/actor.

Actor (main thread) samples on GPU0, learner (background thread) updates
critic/encoder/residual on GPU1.  The threading boundary mirrors
train_pi_robo_async.py exactly.
...
def main(_):
    ...
    # --- Device layout: GPU0 for actor (PyTorch), GPU1 for learner (JAX) ---
    ...
    log_dir = os.path.join(FLAGS.output_dir, FLAGS.run_name or "gr00t-async")
    ...
    # Ensure cache_infer_params puts JAX params onto the sample device so
    # the main thread can use them without cross-device transfers.
    agent.actor.infer_sharding = sample_sharding
    ...
    for i in tqdm.tqdm(
        ...
        if done:
            ...
            # Don't wait 10 episodes to start updating — JAX compilation is slow.
